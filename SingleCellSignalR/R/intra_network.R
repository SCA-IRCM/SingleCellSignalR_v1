...
#' @param write a logical (if TRUE writes graphML and text files for the internal networks)
#' @param plot a logical
#' @param add.lig a logical (if TRUE adds the goi associated ligands from signal to the network)
#' @param connected a logical (if TRUE keeps only the genes connected to the goi)
#' @param verbose a logical
#'
#' @return The function returns a list containing the internal networks linked to the genes of interest (goi)
#'
#' @export
#'
...
intra_network = function(goi,data,genes,cluster,coi,cell.prop=0.2,c.names=NULL,signal=NULL,write=TRUE,plot=TRUE,add.lig=TRUE,species=c("homo sapiens","mus musculus"),connected=FALSE,verbose=TRUE){
  ...
  if (!is.element(coi,c.names)){
    ...
    cat("If c.names is not provided, it is set to cluster 1, cluster 2, ..., cluster N. WIth N the maximum number of clusters",fill=TRUE)
    return()
  }
  opar = par()
  ...
  visible.n = PwC_ReactomeKEGG[PwC_ReactomeKEGG$a.gn%in%visible.genes & PwC_ReactomeKEGG$b.gn%in%visible.genes,]
  ...
  for (receptors in goi){
    ...
    if (!is.element(receptors,rownames(data.tmp))){
      ...
      contains.receptors = intersect(unlist(pw.names[PwC_ReactomeKEGG$a.gn%in%receptors | PwC_ReactomeKEGG$b.gn%in%receptors]),names(good.pw))
      if (verbose == TRUE){
      ...
      contain.n=NULL
      for (i in contains.receptors){
        contain.n = rbind(contain.n,PwC_ReactomeKEGG[grepl(i,PwC_ReactomeKEGG$pathway),])
      }
      contain.n = unique(rbind(contain.n,PwC_ReactomeKEGG[PwC_ReactomeKEGG$a.gn%in%visible.genes & PwC_ReactomeKEGG$b.gn%in%receptors,],PwC_ReactomeKEGG[PwC_ReactomeKEGG$a.gn%in%receptors & PwC_ReactomeKEGG$b.gn%in%visible.genes,])) # in case interactions with receptors are in no pathway
      ...
      if (nrow(net.n)>0){
        ...
        if (is.null(signal)==FALSE & add.lig==TRUE){
          for (i in names(signal)){
            if (grepl(paste0(coi,"$"),i)){
              ...
              if (is.element(receptors,tmp[,2])){
                siz = c(siz,rowMeans(data[tmp[tmp[,2]%in%receptors,1],cluster==as.numeric(which(c.names%in%colnames(tmp)[1]))]))
                nam.tmp = rep(colnames(tmp)[1],nrow(tmp[tmp[,2]%in%receptors,]))
                colnames(tmp)[1:2] = c("a.gn","b.gn")
          ...
          if (is.null(add.net)==FALSE){
            add.net = cbind(add.net[,1:2],nam,location="extra",type="control")
          }
        }
        net.tmp = cbind(net.n[,1:2],nam=rep(coi,nrow(net.n)),location="intra",type=net.n$type)
        ...
        g.net.tmp = graph_from_data_frame(net.f[,1:2],directed=FALSE)
        V(g.net.tmp)$status = "pw.related"
        V(g.net.tmp)$status[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% receptors)] = "gene.of.interest"
        V(g.net.tmp)$status[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% net.f$a.gn[net.f$location=="extra"])] = "ligand"
        ...
        if (sum(net.f$location=="extra")){
          x[net.f$a.gn[net.f$location=="extra"]]=(seq(0,length(net.f$a.gn[net.f$location=="extra"])*4-1,4)-max(seq(0,length(net.f$a.gn[net.f$location=="extra"])*4-1,4))/2)
        }
        x[receptors] = 0
        for (j in 1:max(y)){
          if (sum(y==j)>1){
            x[y==j] = seq(-10,10,19/sum(y==j))[1:sum(y==j)]
          }
        }
        l = cbind(x,-y)
        for (i in 1:max(y)){
          if(sum(y==i)>1){
            index = which(y==i)
            l[index[seq(1,length(index),2)],2]=l[index[seq(1,length(index),2)],2]-0.2
            l[index[seq(2,length(index),2)],2]=l[index[seq(2,length(index),2)],2]+0.2
          }
        }
        ...
        if (sum(net.f$location %in% "intra")==0){
          ...
          V(g.net)$size = (log(c(siz,rowMeans(data.tmp[unique(c(net.n$a.gn,net.n$b.gn)),])))+5)*4
          V(g.net)$size[V(g.net)$size<exp(-5)]=exp(-5)
        }
        V(g.net)$size[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% receptors)] = 35
        V(g.net)$shape = c("circle")
        V(g.net)$shape[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% receptors)] = c("rectangle")
        V(g.net)$color = c("lightcyan")
        V(g.net)$color[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% receptors)] = c("indianred1")
        V(g.net)$color[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% net.f$a.gn[net.f$location=="extra"])] = c("lightyellow1")
        V(g.net)$label.color = "black"
        V(g.net)$label.dist[which(!unique(c(net.f$a.gn,net.f$b.gn)) %in% receptors)] = 1
        for (i in unique(l[,2])){
          if (i!=0){
            V(g.net)$label.degree[l[,2]==i] = rep(c(pi/2,-pi/2),length(y))[1:sum(l[,2]==i)]
          }
        }
        V(g.net)$label.dist[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% receptors)] = 0
        V(g.net)$label.degree[which(unique(c(net.f$a.gn,net.f$b.gn)) %in% receptors)] = 0
        ...
        pw.table = foreach(pw=names(g.pw.sizes),.combine=rbind) %do% {
          ...
          if (K <= max.pw.size){
            ...
            data.frame(pathway=pw,in.pw=k,pw.size=K,pval=pval,stringsAsFactors=F)
          }
          else
            NULL
        }
        ...
        if (is.null(pw.table)==FALSE){
          ...
          if (nrow(pw.table)>0){
            pw.table = pw.table[order(pw.table$in.pw,decreasing = F),]
            nc = max(nchar(as.character(pw.table$pathway)))*0.3

            mtext(paste(receptors,"related pathways"),side = 1,line = 2,las=F)
            par(mar=c(2,nc,4,2))
            barplot((pw.table$in.pw),horiz = TRUE,names.arg = (paste(pw.table$pathway,"*")),
          ...
          if (verbose ==TRUE){
            cat(paste("No associated genes downstream",receptors, "in",coi),fill=TRUE)
          }
        }

        if (write==TRUE){
          if (is.null(pw.table)==FALSE){
            write.table(pw.table[order(pw.table$qval),],file=paste0('./networks/intracell_network_pathway_analysis_',coi,'-',receptors,'.txt'),sep="\t",quote=F,row.names=F)
          }
          write.graph(g.net.tmp,paste0('./networks/intracell_network_',coi,'-',receptors,'.graphml'),format = "graphml")
        }
        res[[qq]] = net.f
      }
      else {
