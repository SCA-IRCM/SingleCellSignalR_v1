...
#' @param tsne (optional) a table of n rows and 2 columns with t-SNE projection coordinates for each cell
#' @param plot.details a logical (if TRUE, then plots the number of cells attributed to one cell type, see below)
#' @param write a logical
#' @param verbose a logical
#'
#' @return The function returns a list containing the thresholded table, the maximum table, the raw table, a cluster vector,
#' a pheatmap object (see pheatmap library) and the cluster names. The maximum table is a special thresholded table where in
#' every column only the maximum gene signature is kept. It can be used to force the classification of every cell. The pheatmap
#' object contains the thresholded matrix, which is the one to be used for most applications, in a heatmap representation for
#' immediate display purpose.
#'
#' @export
#' @import pheatmap
...
cell_classifier = function(data,genes,markers = markers.default,tsne=NULL,plot.details=FALSE,write=TRUE,verbose=TRUE){
  ...
  tmp = data[as.character(unlist(markers))[as.character(unlist(markers)) %in% genes],]
  ...
  final[is.na(final)] = 0
  final[,colSums(final)!=0] = apply(final[,colSums(final)!=0],2,function(x) x/sum(x))
  ...
  res[[2]] = final
  h = pheatmap::pheatmap(res[[1]],cluster_rows = T,cluster_cols = T,show_colnames = F)

  nn=1:length(rownames(res[[1]])[rowSums(res[[1]])!=0])
  names(nn)=rownames(res[[1]])[rowSums(res[[1]])!=0]
  cluster = unlist(lapply(apply(final.s,2,function(x) names(x[x>0])),function(x) if(length(x)==1){return(nn[as.character(x)])} else {return(0)}))
  ...
  d=data.frame(Cell_type=c(rownames(res[[1]]),"undefined"),Number_of_cells=c(apply(res[[1]],1,function(x) sum(x!=0)),ncol(res[[3]])-ncol(res[[1]])))
  d=d[d$Number_of_cells!=0,]
  rownames(d)=paste("cluster",1:nrow(d))


  if (verbose==TRUE){
    cat(paste("A threshold of",round(seuil*1000)/10,"% maximizes the number of cells assigned to one cell type"),fill=TRUE)
    for (i in (nrow(m)):1){
      cat(paste(m[i,2],"cell(s) or",round(m[i,2]*1000/ncol(data))/10,"% identified to",(n.types)-m[i,1],"cell type(s)"),sep = " ",fill=TRUE)
    }
    cat(" ",fill=TRUE)
    print(d)
  }
  ...
  if (is.null(tsne)==FALSE){
    plot(x=tsne[,1],y=tsne[,2],type = 'n',main="t-SNE Map",xlab="t-SNE1",ylab="t-SNE2",xlim=c(min(tsne[,1])*2,max(tsne[,1])*1.05))
    abline(h=0)
    abline(v=0)
    symbols(x=tsne[,1],y=tsne[,2],circles=rep(1,nrow(tsne)),inches=0.04,bg=cr[cluster],add=TRUE)
  ...
  if (write==TRUE){
    ...
    fwrite(data.frame(cbind(rownames(res[[3]]),res[[3]])),"cell-classification/raw_class_matrix.txt",sep="\t")
