...
#' @description Analysis of the differentially expressed genes in the clusters and their composition by a marker based approach.
#'
...
#' @param dif.exp a logical (if TRUE, then computes the diferential gene expression between the clusters using **edgeR**)
...
#'
#' @return The function returns a list comprised of a table of differentially expressed genes, a table of cell types, and a
...
cluster_analysis = function(data,genes,cluster,c.names=NULL,dif.exp=TRUE,s.pval=10^-2,markers=NULL,write=TRUE,verbose=TRUE){
  ...
  if (dif.exp==TRUE){
    ...
    for (i in z){
      if (verbose==TRUE){
        cat(paste("Looking for differentially expressed genes in", c.names[i]),fill=TRUE)
      }
      ...
      if (length(sel.r)==0){
        if (verbose==TRUE){
          cat(paste("No differentially expressed genes in", c.names[i]),fill=TRUE)
        }
        v = c(v,i)
      } else {
        k = k+1
        ...
        diff.genes[[k]] = resu
        if (write==TRUE){
          fwrite(data.frame(resu),paste("./cluster-analysis/table_dge_",c.names[i],".txt",sep=""),sep="\t")
        }
      }
    }
    if (is.null(v)==TRUE){
  ...
  if (is.null(markers)==FALSE){
    ...
    for (i in z){
      ...
      for (j in 1:ncol(markers)){
        ...
        tmp = c(tmp,sum(rowSums(data.frame(data[m.genes,cluster==i]))/sum(cluster==i))/length(m.genes))
        tmp[is.na(tmp)] = 0
      }
    ...
    for (i in z){
      ...
      if (length(c.type)>2){
        if (verbose==TRUE){
          cat(paste(c.names[i], ": Unconclusive (more than 2 cell types attributed to the cluster)",sep=""),fill=TRUE)
        }
        class[[i]] = "Undefined"
      }
    }
    ...
    if (write==TRUE){
      fwrite(data.frame(final),paste("./cluster-analysis/cluster_types.txt",sep=""),sep="\t")
    }
  }
