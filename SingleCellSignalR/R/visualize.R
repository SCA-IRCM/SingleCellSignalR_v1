...
#'
#' @param inter a list of data frames result of the **cell_signaling()** function
...
#'
#' @return The function returns images in the plot window of Rstudio and images in the pdf format in the *images* folder.
#' @export
...
#' visualize(inter)
visualize = function(inter,show.in=NULL,write.in=NULL,write.out=FALSE,method="default",limit=30){
  options(warn=-1)
  if (dir.exists("images")==FALSE & (is.null(write.in)==FALSE | write.out==TRUE)){
    dir.create("images")
  }
  c.names = NULL
  for (i in 1:length(inter)){
    c.names=c(c.names,colnames(inter[[i]])[1:2])
  }
  ...
  for (i in 1:length(inter)){
    if (is.null(dim(inter[[i]]))==TRUE){
      nn=rbind(nn,names(inter[[i]])[1:2])
    } else {
      nn=rbind(nn,colnames(inter[[i]])[1:2])
    }
    s = c(s,nrow(inter[[i]]))
  }
  ...
  if (write.out==TRUE){
    ...
    chordDiagramFromDataFrame(dat, annotationTrack = "grid", preAllocateTracks = 1, directional = 1,
                              direction.type = "arrows",
                              link.arr.length = 0.15,
                              link.arr.width = 0.15,
                              link.arr.type = "triangle", link.arr.lty = par("lty"),
                              link.arr.lwd = par("lwd"), link.arr.col = "black",
                              grid.col = cols,col=cr[score-min(score)+1],big.gap = 5, small.gap = 2)
    ...
    text(coords[1]*0.925,coords[3]*0.97,labels = as.character(max(score)),col = "white")
    text(coords[1]*0.92,coords[4]*1.05,labels = as.character(min(score)),col = "white")
  ...
  for (n in write.in){
    ...
    dat = data.frame(name,feature)
    cluster.col = as.character(c(rep(cols[cnames[2]],length(unique(tmp[,2]))),rep(cols[cnames[1]],length(unique(tmp[,1])))))
    ...
    pdf(paste("./images/",cnames[1],"-",cnames[2],"_",method,"_diagram.pdf",sep=""))
    chordDiagramFromDataFrame(dat, annotationTrack = "grid", preAllocateTracks = 1, directional = -1,
                              direction.type = "arrows",
                              link.arr.length = link.width,
                              link.arr.width = link.width,
                              link.arr.type = "circle", link.arr.lty = par("lty"),
                              link.arr.lwd = link.lwd, link.arr.col = link.col,
                              grid.col = cluster.col,col=cr[score],big.gap = 5, small.gap = 0.7)
    circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
      xlim = get.cell.meta.data("xlim")
      ylim = get.cell.meta.data("ylim")
      sector.name = get.cell.meta.data("sector.index")
      circos.text(mean(xlim), ylim[1] + .1, sector.name, facing = "clockwise", niceFacing = TRUE, adj = c(0, 0.5))
      circos.axis(h = "top",labels = FALSE,minor.ticks = FALSE, major.tick.percentage = 0.2, major.tick.length = 2,major.at = c(xlim), sector.index = sector.name, track.index = 2)
    }, bg.border = NA)
    bx = par("usr")
    coords = c(bx[1]*0.95,bx[1]*0.82,bx[4]*0.94,bx[4]*0.53)
    l = length(cr)
    ...
    text(coords[1]*0.925,coords[3]*0.97,labels = as.character(round(max(as.numeric(tmp[,4]))*100)/100),col = "white")
    text(coords[1]*0.925,coords[4]*1.05,labels = as.character(round(min(as.numeric(tmp[,4]))*100)/100),col = "white")
    text((coords[1]+coords[2])/2,coords[3]*1.03,labels = "score")
    legend("topright",fill = rev(unique(cluster.col)),legend=cnames[1:2])
    dev.off()
  }
  for (n in show.in){
    if (is.null(dim(inter[[n]]))==TRUE){
    ...
    chordDiagramFromDataFrame(dat, annotationTrack = "grid", preAllocateTracks = 1, directional = -1,
                              ...
                              link.arr.type = "circle", link.arr.lty = par("lty"),
                              link.arr.lwd = link.lwd, link.arr.col = link.col,
                              grid.col = cluster.col,col=cr[score],big.gap = 5, small.gap = 0.2)
    circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
      xlim = get.cell.meta.data("xlim")
      ylim = get.cell.meta.data("ylim")
      sector.name = get.cell.meta.data("sector.index")
      circos.text(mean(xlim), ylim[1] + .1, sector.name, facing = "clockwise", niceFacing = TRUE, adj = c(0, 0.5), cex=ifelse(nrow(dat)>50, yes = 0.5,no=1))
