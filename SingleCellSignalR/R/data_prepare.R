...
data_prepare = function(file, most.variables=0, lower=0, upper=0, normalize=TRUE, write=FALSE, verbose=TRUE, plot=FALSE){
  ...
  if (sum(data[,p] %in% c(mm2Hs$`Mouse gene name`,mm2Hs$`Gene name`))==0){
    cat("Please convert gene ID's (Ensembl or NCBI ID) to official HUGO gene symbols",fill=TRUE)
    return()
  }
  ...
  data = data[rowSums(data)>0,]
  data = data[rowSums(data)<quantile(rowSums(data),1-upper) & rowSums(data)>quantile(rowSums(data),lower),]
  ...
  if (most.variables!=0){
    ...
    if (length(cv)<most.variables){
      ...
      mv.genes = names(sort(cv,decreasing = TRUE))[1:most.variables]
    }
    mv.genes = names(sort(cv,decreasing = TRUE))[1:most.variables]
